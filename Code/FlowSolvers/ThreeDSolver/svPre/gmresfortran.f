...
!
     ...
     &         PP, DZ, RWORK, IWORK, AK, BK, BNRM, SOLNRM) .NE. 0 ) THEN
           GO TO 200
         ENDIF
...
!
     ...
     &      RWORK, IWORK, AK, BK, BNRM, SOLNRM) .NE. 0 ) THEN
        GO TO 200
      ENDIF
...
!         check stopping criterion.
     ...
     &         IWORK, AK, BK, BNRM, SOLNRM) .NE. 0 ) THEN
           GO TO 200
         ENDIF 
!
 100  CONTINUE
...
!
     ...
     &      DZ, ATDZ, RWORK, IWORK, AK, BK, BNRM, SOLNRM) .NE. 0 ) THEN
        GO TO 200
      ENDIF
      IF( IERR.NE.0 ) RETURN
!
...
!
     ...
     &      U, V1, V2, RWORK, IWORK, AK, BK, BNRM, SOLNRM) .NE. 0 ) THEN
           GO TO 200
      ENDIF
      IF( IERR.NE.0 ) THEN
        RETURN
...
!         check stopping criterion.
         IF( ISDCGS(N, B, X, NELT, IA, JA, A, ISYM, MATVEC, MSOLVE,
     &         ITOL, TOL, ITMAX, ITER, ERR, IERR, IUNIT, R, R0, P, Q,
     &         U, V1, V2, RWORK, IWORK, AK, BK, BNRM, 
     &         SOLNRM) .NE. 0 ) THEN 
...
!LLL. OPTIMIZE
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      INTEGER  N, NELT, IA(NELT), JA(NELT), ISYM, NSAVE, ITOL, LoopA, tempInt
      INTEGER  ITMAX, ITER, IERR, IUNIT, LENW, LENIW, IWORK(LENIW)
      DOUBLE PRECISION B(N), X(N), A(NELT), TOL, ERR, RWORK(LENW)
      EXTERNAL DSMV, DSLUI
      PARAMETER (LOCRB=1, LOCIB=11)
...
!***END PROLOGUE  DSGS
      ...
      DOUBLE PRECISION B(N), X(N), A(N), TOL, ERR, RWORK(*)
      EXTERNAL DSMV, DSLI
      PARAMETER(LOCRB=1,LOCIB=11)
!
!         Modify the SLAP matrix data structure to YSMP-Column.
...
!***FIRST EXECUTABLE STATEMENT  QS2I1D
      ...
      IF (NN.LT.1) THEN
         CALL XERROR (
     &    'QS2I1D- the number of values to be sorted was not 
     &    positive.',59,1,1)
         RETURN
      end if
      IF( N.EQ.1 ) RETURN
      ...
      IF ( KK.NE.1 ) THEN
         CALL XERROR (
     &      'QS2I1D- the sort control parameter, k, was not 
     &      1 or -1.',55,2,1)
         RETURN
      end if
!
...
!***END PROLOGUE  DSMMI2
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      INTEGER N, IL(*), JL(*), IU(*), JU(*)
      DOUBLE PRECISION B(N), X(N), L(*), DINV(N), U(N)
!
!         Solve  L*Y = B,  storing result in X, L stored by rows.
...
!
     ...
     &      RWORK, IWORK, AK, BNRM, SOLNRM) .NE. 0 ) THEN
        GO TO 200
      ENDIF
      IF( IERR.NE.0 ) THEN
        RETURN
...
!     decide whether to print message
     ...
     &  .or.((llevel.eq.2)   .and.(kount.gt.max0(1,maxmes)))) THEN
       GO TO 100
      ENDIF
...
!
      ...
      write ( string, '(a,1x,i2,1x,i4,2x,i2,a1,i2.2,a1,i2.2,a1,
     &      i3.3,1x,a)' ) 
