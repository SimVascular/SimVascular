...
mitk::svSurface::svSurface()
  : m_CalculateBoundingBox(false)
{
  this->InitializeEmpty();
}

mitk::svSurface::svSurface(const mitk::svSurface& other)
  : BaseData(other),
...
void mitk::svSurface::Swap(mitk::svSurface& other)
{
...
mitk::svSurface& mitk::svSurface::operator=(svSurface other)
{
  this->Swap(other);
  return *this;
}
...
const mitk::svSurface::RegionType& mitk::svSurface::GetLargestPossibleRegion() const
{
...
const mitk::svSurface::RegionType& mitk::svSurface::GetRequestedRegion() const
{
  return m_RequestedRegion;
}

void mitk::svSurface::InitializeEmpty()
{
...
void mitk::svSurface::SetVtkPolyData(vtkPolyData* polyData, unsigned int t)
{
  this->Expand(t + 1);
...
bool mitk::svSurface::IsEmptyTimeStep(unsigned int t) const
{
  if(!IsInitialized())
    return false;

  vtkPolyData* polyData = const_cast<svSurface*>(this)->GetVtkPolyData(t);
...
vtkPolyData* mitk::svSurface::GetVtkPolyData(unsigned int t) const
{
...
void mitk::svSurface::UpdateOutputInformation()
{
...
void mitk::svSurface::CalculateBoundingBox()
{
...
void mitk::svSurface::SetRequestedRegionToLargestPossibleRegion()
{
  m_RequestedRegion = GetLargestPossibleRegion();
}

bool mitk::svSurface::RequestedRegionIsOutsideOfTheBufferedRegion()
{
...
bool mitk::svSurface::VerifyRequestedRegion()
{
...
void mitk::svSurface::SetRequestedRegion(const itk::DataObject* data )
{
  const mitk::svSurface *surface = dynamic_cast<const mitk::svSurface*>(data);

  if (surface != nullptr)
    m_RequestedRegion = surface->GetRequestedRegion();
  else
    mitkThrow() << "Data object used to get requested region is not a mitk::svSurface.";
}

void mitk::svSurface::SetRequestedRegion(svSurface::RegionType* region)
{
  if (region == nullptr)
...
void mitk::svSurface::CopyInformation(const itk::DataObject* data)
{
  ...
  if (surface == nullptr)
    mitkThrow() << "Data object used to get largest possible region is not a mitk::svSurface.";

  m_LargestPossibleRegion = surface->GetLargestPossibleRegion();
}

void mitk::svSurface::Update()
{
  using ::Update;
...
void mitk::svSurface::Expand(unsigned int timeSteps)
{
...
void mitk::svSurface::ExecuteOperation(Operation* operation)
{
...
unsigned int mitk::svSurface::GetSizeOfPolyDataSeries() const
{
  return m_PolyDatas.size();
}

void mitk::svSurface::Graft(const DataObject* data)
{
  const svSurface* surface = dynamic_cast<const svSurface*>(data);

  if(surface == nullptr)
    mitkThrow() << "Data object used to graft surface is not a mitk::svSurface.";
  ...
  for (unsigned int i = 0; i < surface->GetSizeOfPolyDataSeries(); ++i)
  {
    ...
    m_PolyDatas.back()->DeepCopy(const_cast<mitk::svSurface*>(surface)->GetVtkPolyData(i));
  }
}

void mitk::svSurface::PrintSelf(std::ostream& os, itk::Indent indent) const
{
...
bool mitk::Equal( mitk::svSurface* leftHandSide, mitk::svSurface* rightHandSide, mitk::ScalarType eps, bool verbose )
{
  if(( leftHandSide == nullptr ) || ( rightHandSide == nullptr ))
  {
    MITK_ERROR << "mitk::Equal( mitk::svSurface* leftHandSide, mitk::svSurface* rightHandSide, mitk::ScalarType eps, bool verbose ) does not work with NULL pointer input.";
    return false;
  }
  return Equal( *leftHandSide, *rightHandSide, eps, verbose);
}
