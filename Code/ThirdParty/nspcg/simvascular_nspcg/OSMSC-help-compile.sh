...
if [ ${USE_32BIT_COMPILERS:-NO} == 'YES' ] ; then
  ...
  mkdir -p /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x86/nspcg
  chmod a+rx /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x86/nspcg
  make -f Makefile.windows.osmsc clean
  make -f Makefile.windows.osmsc
  cp libnspcg.lib /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x86/nspcg
  chmod -R a+rx /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x86/nspcg
  make -f Makefile.windows.osmsc clean
else
  export USE_32BIT_COMPILERS=NO
  echo "build 64-bit libraries (${USE_MSVC_COMPILERS})"
  mkdir -p /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x64/nspcg
  chmod a+rx /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x64/nspcg
  make -f Makefile.windows.osmsc clean
  make -f Makefile.windows.osmsc
  cp libnspcg.lib /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x64/nspcg
  chmod -R a+rx /sv_extern/bin/win/${USE_MSVC_COMPILERS}/x64/nspcg
  make -f Makefile.windows.osmsc clean
fi
