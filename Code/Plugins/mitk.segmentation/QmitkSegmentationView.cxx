...
#include "QmitkSegmentationOrganNamesHandling.cxx"
...
const QString QmitkSegmentationView::EXTENSION_ID = "mitk.segmentation";
...
//     SetActivated(true);
   ...
   if( m_Controls )
   {
      ...
      mitk::DataStorage::SetOfObjects::ConstPointer image = this->GetDataStorage()->GetSubset( m_IsNotABinaryImagePredicate );
      if (!image->empty()) {
...
//      this->SetToolManagerSelection(0,0);
      ...
      us::ModuleContext* context=us::ModuleRegistry::GetModule("MitkSegmentation")->GetModuleContext();
      us::ServiceReference<mitk::PlanePositionManagerService> serviceRef =
          context->GetServiceReference<mitk::PlanePositionManagerService>();
      mitk::PlanePositionManagerService* service = context->GetService(serviceRef);
      service->RemoveAllPlanePositions();
//      context->UngetService(serviceRef);
      this->SetToolManagerSelection(0,0);
...
void QmitkSegmentationView::OnPatientComboBoxSelectionChanged( const mitk::DataNode* node )
{
   ...
   if( node != NULL )
   {
      ...
      if (segNode)
      {
         mitk::DataStorage::SetOfObjects::ConstPointer possibleParents = this->GetDataStorage()->GetSources( segNode, m_IsNotABinaryImagePredicate );
...
void QmitkSegmentationView::OnSegmentationComboBoxSelectionChanged(const mitk::DataNode *node)
{
   ...
   {
      mitk::DataStorage::SetOfObjects::ConstPointer possibleParents = this->GetDataStorage()->GetSources( node, m_IsNotABinaryImagePredicate );
...
void QmitkSegmentationView::OnSelectionChanged(const QList<mitk::DataNode::Pointer>& nodes)
{
   if (nodes.size() != 0)
   {
...
void QmitkSegmentationView::OnContourMarkerSelected(const mitk::DataNode *node)
{
   ...
   QmitkRenderWindow* RenderWindow1 =
      this->GetDisplayWidget()->GetRenderWindow1();
   QmitkRenderWindow* RenderWindow2 =
      this->GetDisplayWidget()->GetRenderWindow2();
   QmitkRenderWindow* RenderWindow3 =
      this->GetDisplayWidget()->GetRenderWindow3();
   QmitkRenderWindow* RenderWindow4 =
      this->GetDisplayWidget()->GetRenderWindow4();
...
void QmitkSegmentationView::ForceDisplayPreferencesUponAllImages()
{
   ...
   {
      // iterate all images
      mitk::DataStorage::SetOfObjects::ConstPointer allImages = this->GetDataStorage()->GetSubset( m_IsABinaryImagePredicate );
...
//      node->SetProperty( "volumerendering", mitk::BoolProperty::New( this->GetPreferences()->GetBool("volume rendering", false) ) );
      ...
      node->SetProperty( "outline binary", mitk::BoolProperty::New(true) );
      node->SetProperty( "outline width", mitk::FloatProperty::New( 2.0 ) );
...
void QmitkSegmentationView::CreateQtPartControl(QWidget* parent)
{
   ...
   SetMultiWidget(GetDisplayWidget());
}
